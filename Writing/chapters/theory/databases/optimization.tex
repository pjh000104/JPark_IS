...
but the most common ones are b+trees. B-trees are somewhat similar to the binary search trees.
...
\bPlusTreeFigure
B+trees store data only in the leaf node. The other nodes store keys only for navigation, like the explanations on the internal node 
...
Many ORM frameworks have lazy loading as the default, and only call the related data when it is accessed.
...
This is the opposite of eager loading. This reduces the need for additional queries and increases the efficiency when fetching data. Most ORMs
