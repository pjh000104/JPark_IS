\section{Database Design and Evolution} \label{sec:db design and evolution}
The design and querying of databases has the largest impact on the performance of the database.
This section investigates strategies for database design to increase performance and efficiency,
...
During this process, redundant data is reduced to improve data quality and optimize database performance.
There are several levels of normalization, and as the normalization form gets higher,
it indicates a more refined relational design. However, most databases tend to require up to the third normal form,
which avoids most of the problems common in bad relational designs \cite{Harrington_2016}.\\
...
Here, the repeating groups mean that an attribute has more than one value in each column of a row \cite{Harrington_2016}. 
...
The Table \ref{tab:order_before_1nf} shows the example of a table before the first normalized form. We can see that students have multiple courses in the same column separated by a comma.
...
As we can see from the Table \ref{tab:order_after_1nf} above, there is only one course in the course column by creating a new record for each item.
...
would require updating other duplicate records. Missing any of the records with the student name will result in data inconsistency. 
...
% (RDDI chapt7). 
In other words, all other columns except the columns of the primary key or keys, are dependent on the primary key or the candidate key.
...
For example, in the student Table \ref{tab:order_before_2nf}, the student name and birthday would be dependent on the student id, so the student id will be the primary key.
The courses the student takes do not depend on the student id, but rather on the course id.
We could create another table using the course id as the primary key and the course name as the functional dependencies, resulting in Table \ref{tab:order_after_2nf}.
...
\begin{itemize}
    \item \textbf{Student}(\textit{student\_id} (PK), \textit{course\_id} (FK), \textit{student\_name}, \textit{birthday}, \textit{room\_num}) 
    \item \textbf{Courses}(\textit{course\_id} (PK), \textit{course\_name})
\end{itemize}
...
Going back to this example, we can see that a non-key attribute in the \textbf{Student} Table \ref{tab:order_before_3nf}, (\textit{room\_num}) depends on another non-key attribute (\textit{course\_id}).  

To remove transitive dependencies, we should break the relations into separate tables. In this case, we can:

\begin{itemize}
    \item Remove the \textit{room\_num} column from the \textbf{Student} Table \ref{tab:order_before_3nf} to remove the relationship between non-key attributes.
...
Applying either method will resolve the anomalies we encountered in 2NF and result In Table \ref{tab:after_3nf}.
...
Here is an example how the leader and follower replication setup could look like:

\leaderFollowerFigure

The leader and follower structure has one lead node (leader) that allows writes to the database,
and the rest of the nodes are assigned as followers shown in Figure \ref{fig:leader follower}. If there is a change data in the leader, the followers are updated accordingly.
...
One downside of the leader and follower replication was that write queries were only allowed in the leader replica.
...
In situations with a multiple datacenter operation, multi-leader replication allows each datacenter to process write requests locally,
which reduces latency and improve availability during datacenter or network failures. Also, for applications that support offline operation, 
...
For instance, if the same record is modified by multiple users on different leader replicas, 
...
One way could be to simply give a unique ID to each write queries, and if there is a conflict, 
the higher unique ID overwrites the conflict.
Another way is similar, but instead give each replica node a unique ID, and the higher unique ID overwrites the conflict.
...
Although the process of replicating the leader’s write operations to the database replicas are fast, which usually takes less than a second \cite{Kleppmann_2017},
...
This is an advantage of synchronous replication, as it ensures that all replicas remain up-to-date with the leader database.
However, it can also be a disadvantage, as it slows down query processing.
Most of the times,
...
up-to-date with all the write queries, and the rest of the followers being asynchronous. This configuration is sometimes also called semi-synchronous \cite{Kleppmann_2017}.
...
However, it does have a disadvantage that all of the followers might not be up-to-date, which is also known as replication lag.
...
If the application that is being created tolerates the possibility of inconsistency due to replication lag,
...
replication should be used for the leaders and followers method. 


\subsubsection{Partitioning}

Partitioning is the process of dividing a database’s data into multiple smaller partitions, each containing a subset of records, rows, or documents.
Replication is used for scalability and performance, but as the size of the database increases,
relying solely on replication becomes inefficient since all the data has to be copied to the replicas.
...
Using key-value data is one way to create partitions. The reason for partitioning is to scale the database by distributing the query load
across multiple nodes instead of concentrating it on a single node.
One way to achieve this is to have a key range for each partition. For example, when we think of a dictionary,
we know that words starting with the letter “a” is in the beginning, the letter “m” is somewhere in the middle, and the letter “z” is at the end.
...
Partitioning by key-value pairs work nicely with the partitioning strategies discussed above. However, things get more complicated with secondary indexes.
Secondary indexes are created to improve the query performance that are based on multiple keys. For example, when searching for a job in a job board, selecting criteria such as industry or experience level could be a situation to use secondary indexes.
...
It is also possible to partition secondary indexes by a term in the job board.
...
After partitioning the dataset into multiple nodes, it is important to make sure the request connects to the right partition.
At a high level there are three main ways to achieve this. 
\begin{enumerate}
...
The key problem is how the component, (which could be the routing tier, the client, or the partition in the node)
knows about changes in the other partitions in other nodes. If the deciding component is not up-to-date with the location where data that the client is requesting,
this could lead into data inconsistency. 

To solve this problem, many distributed data systems use a separate coordination service such as ZooKeeper, which registers each node
and maintains the authoritative mapping of partitions to nodes, to keep track of the data \cite{Kleppmann_2017}.
Whenever a partition changes or a node is added or removed ZooKeeper is updated to keep data consistency. 
