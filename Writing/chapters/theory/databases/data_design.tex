...
In this section, we will be investigating strategies to design the database to increase performance and efficency,
consider scalability and evolution of databases, and ways to optimize the database and queries.
...
Here, the repeating groups mean that if an attribute has more than one value in each row of a table \cite{Harrington_2016}. 
...
Having a range of secondary indexes partitioned in separate partitions make read queries more efficient 
from the scatter/gather technique. If write queries are used more often, it is better to use the scatter/gather technique, 
and if read queries are dominantly used, partitioning indexes by term is the structure to choose. \\
