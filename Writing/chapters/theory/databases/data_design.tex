...
Using a key-value data is one way to create partitions. The reason for partitioning is to scale the database by distributing the query load on one node.
...
The key problem is that how the component, (which could be the routing tier, the client, or the partition in the node)
