...
In case different technology stacks are required for separate domains, it would not be possible as all components are contained within a single codebase and deployment unit.
This also connects to decrease in reliability. A single bug or error occurred in the application could degrade the service and affect the whole application.
...
Building a large application using layered monolithic architecture could end up in a confusing relationship between layers and result in challenges maintaining and scale the application.
...
Some applications using the layered architecture may have different structure from the original presentation, workflow, persistence, and database layer.
Some layers might be separated,
...
decreasing the flexibility to reuse the code across modules compared to the modular monolithic structure \cite{Gandhi_Richards_Ford_2024}. Also, similar to other modular monolithic architecture
