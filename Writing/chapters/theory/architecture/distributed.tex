...
This architecture style encourage loose coupling of each service, which could be a huge benefit.
...
loosely coupled, they could be tested separately. This becomes a huge advantage as the size of the application grows compared to
...
For security and authentication, it is important to have these access control in a single microservice,
...
Furthermore, if a single request requires separate microservices to communicate with each other, this request is coupled and recommended to kept in the same service. \\
...
This allows to handle errors efficiently and easy to change a workflow.

The disadvantage is that tight coupling between the central microservice and the other microservices.
...
The advantages of the shared service are that even though a code is changed in the shared microservices, code in other microservices are not needed to be changed.
