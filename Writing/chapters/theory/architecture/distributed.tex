\section[Distributed Systems]{Distributed Systems}\label{sec:distributed_systems}
...
There are multiple advantages when choosing to implement distributed systems. The one that contrasts with the monolithic architecture is
...
This leads to more complexity in the system, and makes it more challenging for the developers to understand the overall design.
Additionally, errors can occur in any unit involved in servicing a request, as logical components are deployed in separate units, making tracing errors more complicated.
...
Microservices architecture is a subcategory of distributed system architecture, where microservices communicate with each other to form an application.
A microservice is a service that is a separately deployed unit of software that performs business or infrastructure process \cite{Gandhi_Richards_Ford_2024}.
...
Here, cohesiveness is based on how close the parts of the application are related to each other \cite{Gandhi_Richards_Ford_2024}.
...
so it does not get too complicated when managing the information. Finally, if one part of the microservice changes,
...
On the other hand, there are certain situations when it is encouraged to make the microservices bigger: database transactions, data dependencies, and workflow.
...
and where workflow is integrated.
A workflow refers to two or more microservices called for a single request.
...
On the other hand, this pattern is lacking in unified error handling, since each microservice is responsible for managing the error workflow,
which could lead to excessive communication between services. Also, because there is no central microservice that has the order
of microservices that is need to be called, it is hard to tell the current state of the response, which decreases the ability to recover
when the request is delayed and needs to be restarted.\\


\noindent \textbf{Sharing functionalities}

In a microservice architecture, shared functionality often needs to be reused across multiple services, which can be achieved in two main ways:
creating a shared service or a shared library.
A shared service is a separate microservice that contains shared functionality that other microservices can call remotely.
...
A shared library is a more common method of code reuse. A library will be built, including all the code that is reused in different microservices.
...
\noindent \textbf{Advantages and Disadvantages of Microservice Architecture.}
...
This architecture is complex. It requires numerous architectural decisions that must be made based on the specific context,
including how workflows are orchestrated and how shared code is managed.
For example, decisions may vary based on the expected service scale, team organization, or the need for independent deployment cycles.
Because of its complexity, as microservices communicate with each other, the performance also decreases.
...
Event-Driven Architecture (EDA) is a subcategory of microservices architecture that enables communication between services through events.
In computer science, an event is a way for a service to let the rest of the system know that something important has just happened.
In EDA, events are defined as passing information to other services \cite{Gandhi_Richards_Ford_2024}.
...
However, events are asynchronous, which will be discussed in more detail in the following paragraph.
...
which means that the service in response must be available. This makes tighter coupling between services, making it not suitable for EDA. \\

\noindent \textbf{Advantages and Disadvantages of EDA}
...
Similar to the microservices architecture, EDAs are complex. Deciding which database topology for the architecture,
