...
As shown in figure \ref{fig:cache-aside}, the client first checks the cache to see if the read request data is in the cache.
...
Whenever there is a read request from the application, the application first checks the cache.
...
The write-back strategy is similar to the write-through strategy, but the writes to the database happen with a delay. The cache is still in charge of writing data to the database.
