...
There are multiple database caching strategies that can be applied when designing cache systems.
Depending on the type of request (read, write) from the user, and which part of the application is responsible for fetching data from the database or managing the cache,
...
Cache Aside is also called Lazy Loading. The application is in control of managing the cache.
...
\noindent As shown in Figure \ref{fig:cache-aside}, the client first checks the cache to see if the read request data is in the cache.
...
After that, the client transfers the data to the cache, so there can be a cache hit the next time the data is needed.
This will increase the performance of the application when the same data is requested multiple times.
...
since the application can just check the cache instead of querying the database.
...
In the read-through strategy, the cache level manages fetching data from the database.
...
\noindent Whenever there is a read request from the application, the application first checks the cache.
If there is a cache hit, it simply returns the data back to the application as shown in Figure \ref{fig:read-through}. If there is a cache miss, the cache level fetches the data from the database.
Since the cache manages fetching data, it simplifies the application logic when retrieving data compared to when the application handles fetching data.
...
\noindent Nothing happens when there is a read request from the application, but when there is a write request,
the data will first be written in the cache and then into the database as shown in Figure \ref{fig:write-through}.
This ensures data consistency when paired with the read-through strategy.
One downside of this strategy is that since it writes to both the cache and the database layer, the latency of the request increases.
...
Figure \ref{fig:write-back} shows how the write-back strategy works. This strategy is similar to the write-through strategy, but the writes to the database happen with a delay. The cache is still in charge of writing data to the database.
...
\noindent Since the cache only writes to the database after a certain amount of time,
...
\noindent The difference between the write-around strategy and the cache-aside strategy is that when there is a write request, only the data in the database is updated.
The data in the cache is only updated when there is a cache miss from a read request as shown in Figure \ref{fig:write-around}. This way, the cache is not overflooded with unnecessary data. The disadvantage of this strategy is that recently written data will always result in a cache miss. This is why the write-around strategy is suitable when the application has a lot of read requests and data is rarely updated.

\section[Redis]{Redis}
% https://learning.oreilly.com/library/view/redis-in-action/9781617290855/kindle_split_011.html
Until now, we have explored the types of database caching strategies. But how are these strategies actually implemented?
...
Reliability is another reason Redis is chosen for database caching. Redis provides persistence options (Relational DB snapshots and append-only file (AOF) logs, 
where AOF logs record every write operation by appending it to a log file for later recovery)
