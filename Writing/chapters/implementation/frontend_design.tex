...
The frontend of the Local Cuisine App is responsible for accurately displaying the data sent from 
...
export async function getCuisineByName(cuisineName: string) {
  try {
    const res = await fetch(
      `${API_BASE_URL}/api/cuisines/` +
      `${encodeURIComponent(cuisineName)}`,
      { credentials: "include" }
    );
...
In Listing \ref{lst:getCuisineByName}, the function constructs the URL dynamically using the provided cuisineName parameter (line 3--7), 
performs a fetch request including credentials, checks for a successful response (line 8), and returns the JSON data (line 10). 
Any errors during the request are caught and logged (lines 11--13), ensuring consistent error handling throughout the frontend. 
...
As discussed in section \ref{sec:clientside_caching}, client-side caching can significantly improve performance 
...
As described in section \ref{sec:clientside_caching}, local storage enables persistent storage of lightweight, non-sensitive data on the client device 
...
This reflects a design consideration that balances performance optimization with data freshness, as discussed in section \ref{sec:clientside_caching}.
