...
Multiple options are available when selecting a web framework for backend development. Java Spring Boot is chosen for this project for mainly two reasons.
...
When I was choosing my framework at the beginning of the project, I was planning to go back to South Korea after graduation.
...
web frameworks, such as Django and Ruby on Rails, were used in South Korea, but Java was the most dominant, making it practical and relevant to the industry.
...
particularly in terms of modular design. This architectural compatibility is discussed in greater detail in a later subsection. 
...
logic rather than database maintenance and infrastructure management. Redis was selected as the caching solution due to its widespread industry adoption,
...
monolithic architectures were a fit, and all software developed or contributed to during internships also employed a monolithic architecture.
...
and also gain experience relevant to future career goals. Developing an application using both monolithic and distributed architectural styles provides valuable
...
However, monolithic architecture was ultimatley chosen for this project. There are a few reasons behind this decision.
...
Through the study of software architecture theory, it became clear that adopting a distributed system architecture at an early stage would introduce unnecessary complexity.
The benefits that I would gain by setting up the distributed system architecture outweigh the drawbacks.

Scalability is one of the primary advantages of a distributed system architecture. If the local cuisine application were to grow significantly and attract thousands of daily users,
a distributed architecture could provide clear benefits in handling increased traffic and workload. However, there is no guarantee, and it is not certain
whether the local cuisine application would need this level of scalability in the future.

While monolithic architectures are often considered less scalable than distributed systems,
they still offer various strategies for improving scalability. Techniques such as horizontal scaling, caching,
and efficient resource management can be applied within a monolithic architecture to accommodate moderate growth,
making it a viable choice for the current scope of the project.
...
Lastly, the biggest reason the monolithic architecture was chosen was the cost. Although a distributed system architecture offers several advantages,
...
\noindent \textbf{Which Monolithic}
...
The modularity of the modular monolithic architecture is well-suited to this application, since the local cuisine application
...
could be viewed as an overkill. There are only three modules for the local cuisine application, and it is possible to 
