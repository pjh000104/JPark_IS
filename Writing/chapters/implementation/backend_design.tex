...
Multiple design approaches were considered when deciding whether the Local Cuisine App should adopt a monolithic architecture or a distributed system architecture.
...
However, as I continued exploring architectural style for the Local Cuisine App, the monolithic architecture seemed to be a more compelling option.
...
The benefits that I would gain by setting up the distributed system architecture outweigh the drawbacks, so it was ultimately not chosen for this project.
Instead, a monolithic architecture was chosen, specifically the modular monolithic architecture.

First, modular monolithic architecture is appropriate for the Local Cuisine App compared to the distributed system architecture.
It is possible that the Local Cuisine App could grow significantly and attract thousands of daily users,
...
average users is in the thousands which can be handled by, the modular monolithic architecture is easier to maintain than a distributed application.  

Furthermore, as the Local Cuisine App is still relatively small, it would not benefit from the loose coupling that the distributed system architecture has.
While it is possible to divide each part of the application into different services, such as having an individual service for cuisines, regions, and reviews,
introducing this level of separation is unnecessary.
The distributed architecture approach is appropriate only when the application scales significantly and requires independently evolving services.
For the present scope of the Local Cuisine App, the current feature set doesn't warrant the added complexity
such as inter-service communication and associated network latency.

Another big reason the monolithic architecture was chosen was the cost. 
Although a distributed system architecture offers several advantages,
the associated infrastructure costs are critical. Deploying and maintaining 
multiple servers to support distributed services can rapidly increase the cost,
that exceeds the scope of a personal project.
For example, a small Amazon EC2 instance such as t3.small, which provides 
2 vCPUs and 2 GB of memory, costs approximately \$0.0208 per hour 
(about \$15 per month) when running continuously according to Amazon Web 
Services EC2 pricing \cite{Amazon_Web_Services}. 
A minimal distributed deployment consisting of several backend instances, 
caching services, and a load balancer could therefore exceed 
\$75--\$100 per month in infrastructure costs.
On the other hand, a monolithic architecture can be deployed on a single 
server instance, keeping the infrastructure cost close to \$15 per month.
This option was more cost-effective and easier to maintain over the long term.
...
the overall requirements and constraints of the project led to the selection of a monolithic architecture for the Local Cuisine App. 

Overall, the modular monolithic architecture was chosen since it preserves the possibility of future evolution into a distributed system architecture.
In case the Local Cuisine App needs to scale, the existing modular structure provides two options: maintain the current monolithic deployment, 
...
For the Local Cuisine App, I decided to use the modular monolithic approach.
...
The application is composed of three main components: regions, which allow users to explore the geographic areas supported by the Local Cuisine App;
...
The modular architecture is well-suited to this application. If this were implemented using a layered architecture,
...
The modular monolith not only mirrors the real-world organization of the application
but also improves maintainability by isolating changes to individual modules
...
of the layered architecture. Here is the structure of the modular monolithic architecture for the Local Cuisine App.

\input{myFigures/FiguresCommand.tex}
\directoryTreeFigure

The Figure \ref{fig:directoryTree} shows three modules in the Local Cuisine App: the region, cuisine, and reviews.
...
could be viewed as excessive. There are only three modules for the Local Cuisine App, and it is possible to 
...
The database for the Local Cuisine App is designed to store three main pieces of information: the region, the cuisine, and the reviews.
Accordingly, there are three main tables: the regions table, the cuisines table, and the reviews table. 
As shown in the entity relationship diagram Figure \ref{fig:erd}, the regions table has a one-to-many relationship with the cuisines table, 
...
To enforce the integrity of databases, normalization the data is crucial.
In the Local Cuisine App's database, each table satisfies the requirements of 3NF.
...
The third normalization form for the database ensures that redundant storage of
...
that the Local Cuisine App requires. For example, some APIs and datasets did not include the original region of the cuisine, or the description of the cuisine.
For the following reasons, the Wikepedia API was chosen since it included all the information needed for the Local Cuisine App, and was able to gather information
efficiently.
...
When testing the API endpoints of the Local Cuisine App, there weren't any significant performance issues when 
...
The Local Cuisine App is designed such that the cuisines and the reviews are searched the most and make database queries.
...
Among the database optimization and the evolution techniques discussed in the theory section \ref{sec:dbOptimization}, indexing is used for the 
...
Another optimization done in the Local Cuisine App is avoiding the N+1 problem.
As mentioned in section~\ref{subsec:nplusone}, the N+1 query problem is a common performance issue in ORM-based applications, and this application uses the Hibernate ORM.
When fetching the cuisines, the cuisine repository uses the following methods.
...
However, the Local Cuisine App avoids this entirely by performing all filtering and joining at the repository level. 

For example, the service method:

\begin{verbatim}
public List<String> getCuisinesByRegion(String regionName) {
    List<Cuisine> cuisines = cuisineRepository
                            .findByRegion_RegionName(regionName);
...
Furthermore, batch fetching was also not necessary in this design because the repository query already fetches all required data in a single call. 
...
Most queries in the Local Cuisine App are centered around a single entity, such as retrieving cuisines by region or reviews by cuisine.
...
By distributing reviews across partitions, queries can be processed parallel, and improves overall query performance as well.
Four partitions specifically are chosen in consideration of the size of the application,
as it distributes records across multiple partitions while avoiding the additional complexity and management overhead associated with a larger number of partitions.
A smaller number of partitions would result in larger partitions and reduce the performance benefits of partitioning.

At the current stage of development, secondary indexes were not introduced into the database design.
...
Caching is implemented in the Local Cuisine App to enhance performance and 
reduce latency for frequently accessed data.  
Caching in the Local Cuisine App improves performance by reducing database access for frequently requested data.
...
Cuisine data and the reviews data implemented the caching strategy in the Local Cuisine App.
Among the caching strategy discussed in the theory section, the cache-aside pattern is used for the cuisine data.
As the cuisine data won't be written or updated by the users, all the caching strategies that involved writing was not applicable,
...
The CuisineService class checks the cache before querying the database and stores the resultyh
in the cache on a cache miss. This ensures that subsequent requests for the same region
can be served directly from memory.


\begin{lstlisting}[caption={CuisineService -- Cache-aside implementation for getCuisinesByRegion}, label={lst:cuisineCacheByRegion}, numbers=left, firstnumber=1]
public List<String> getCuisinesByRegion(String regionName) {
...
The caching process follows the cache-aside pattern and is implemented in three steps as shown in Listing \ref{lst:cuisineCacheByRegion}.
First, a unique cache key is generated using the requested region name (line 3). This ensures that
each region’s cuisine list is cached independently.

Next, the application queries the cache using the generated key (lines 6--9). If the data exists
...
using the CuisineRepository (lines 12--13). The retrieved cuisine entities are then transformed
into a list of cuisine names (lines 15--17). Finally, the result is stored in the cache with time-based invalidation of one hour (line 20) before being returned to the client.
...
and purpose. However, This also ensures consistent performance improvements when accessing across different cuisines by the cuisine name.

However, the createReview method was initially designed to use a write-through caching strategy to keep the review cache consistent with the database at all times.
Under this approach, each write operation directly updates the cached review list, ensuring strong consistency between the cache and the database.
The original implementation of this strategy is shown in the following listing.

\begin{lstlisting}[caption={ReviewService -- Write-through caching example}, 
                   label={lst:reviewWriteThroughCache}, 
                   numbers=left, 
                   firstnumber=1,
                   breaklines=true,
...
We can see that a created review is both written in the cache and the database (lines 12, 26).  
...
\begin{lstlisting}[caption={ReviewService -- Cache-aside implementation for createReview}, label={lst:reviewCacheAside}, numbers=left, firstnumber=1, breaklines=true,
                   breakatwhitespace=true]
public ReviewResponse createReview(CreateReviewRequest request) {
...
The difference from the write-through database caching method is that new reviews are now only written in the database (line 12), and the existing cache is evicted (line 15).  
...
In the case of the Local Cuisine App, the only meaningful domain event related to reviews occurs when a new review is created for a cuisine.
...
\begin{lstlisting}[caption={ReviewCreatedEvent -- domain event}, label={lst:reviewCreatedEvent}, numbers=left, firstnumber=1]
...
\begin{lstlisting}[caption={ReviewEventListener -- event-based cache invalidation}, label={lst:reviewEventListener}, numbers=left, firstnumber=1]
@Component
...
However, in this system the cache eviction already occurs directly within the review creation workflow (line 15 in Listing~\ref{lst:reviewCacheAside}).
Because the system has a single, well-defined write path for reviews, introducing a full event-based invalidation mechanism would add unnecessary complexity to the application. 
...
The following sequential diagram Figure \ref{fig:sequential-diagram} shows the lifecycle of a backend request starting from a user. 
...
As shown in the sequential diagram Figure \ref{fig:sequential-diagram}, the workflow begins when the user initiates a search action from the frontend interface.
...
\begin{lstlisting}[caption={CuisineController -- Handling incoming HTTP requests}, label={lst:cuisineController}, numbers=left, firstnumber=1, breaklines=true,
                   breakatwhitespace=true]
@RestController
...
As shown in Listing~\ref{lst:cuisineController}, the controller serves as the entry point 
for requests originating from the frontend. When a user action triggers an HTTP request, 
it is routed to this class through the defined endpoint mappings (lines 11--12).

The entry into the backend occurs through the methods annotated with @GetMapping 
(line 11). For example, the endpoint "/byRegion/{regionName}" (line 11) receives a 
request from the frontend and directs it to the method getByRegion (line 12). At this 
stage, the controller extracts the request parameter using @PathVariable String 
regionName (line 12), which binds the value provided in the URL to the method argument. 
After extraction, the controller immediately forwards the parameter to the service 
layer by invoking cuisineService.getCuisinesByRegion(regionName) (line 13). 

The controller contains no business logic, ensuring a clear separation of the layers.
The CuisineService contains the business logic for retrieving a cuisine by name. It first checks the cache using the
cache-aside pattern.

\begin{lstlisting}[caption={CuisineService -- Cache-aside lookup}, label={lst:cuisineServiceCache}, numbers=left, firstnumber=1, breaklines=true, breakatwhitespace=true]
...
If the cache contains a valid entry (lines 6--12), it is converted into a CuisineResponse 
using ObjectMapper (line 9), which is a uitility class of a Java library used for mapping Java objects
and JSON-compatible structures.
In this context, it transforms the cached generic object retrieved from Redis (line 6) back to the CuisineRepsonse type.
This will be explained more in detailed in the next step.
This avoids unnecessary database access and reduces response time.

\begin{lstlisting}[caption={CuisineService -- Database fallback}, label={lst:cuisineServiceDb}, numbers=left, firstnumber=1, breaklines=true, breakatwhitespace=true]
...
Here, the repository retrieves the Cuisine entity by name (lines 1--3). If there are no matching entity, 
an exception is thrown (line 3) and and propagated back to the controller.
The controller handles the exception and returns an appropriate HTTP error response to the frontend, 
typically a 404 Not Found status with a descriptive error message.
The error message is sent to the frontend, and the user is informed them that the requested cuisine could not be found.
If the cuisine data is found in the database that matches the cuisine name, the response is converted 
into a CuisineResponse object (line 5), which is an object that is 
responsible for transferring data to the frontend.

\begin{lstlisting}[caption={CuisineService -- Caching database result}, label={lst:cuisineServicePut}, numbers=left, firstnumber=1]
cacheService.put(cacheKey, response, 3600);
return response;
\end{lstlisting}

The response is cached with a time invalidation of 3600 seconds (1 hour) in Redis (line 1).
This particular duration was chosen as a balance between performance and data freshness. 
It allows frequently requested cuisine data to be served from the cache, reducing 
database load and improving response times, while ensuring that any updates to 
cuisine information in the database are reflected to users within a reasonable 
timeframe. A shorter time invalidation could result in excessive cache misses and higher 
database queries, and a longer one could serve outdated data for extended periods. 
Finally, the controller returns the CuisineResponse to the frontend (line 2), which displays it to the user.
Below is the full FindCuisineByName method.

\begin{lstlisting}[caption={CuisineService -- Caching database result}, label={lst:cuisineServicePut}, numbers=left, firstnumber=1, breaklines=true, breakatwhitespace=true]
cacheService.put(cacheKey, response, 3600);
public CuisineResponse getCuisineByName(String cuisineName) {
...
In the previous subsection, we explored the workflow for a backend request. This section will introduce in depth
what happens within the Java classes and how they interact with each other
to enable those workflows as a part of the modular monolithic architecture. 

The following UML class diagram Figure \ref{fig:class-diagram} provides a visual representation of the structural organization of the Local Cuisine App.
...
\noindent \textbf{Overall System Structure}


The Local Cuisine App is organized into three main modules, and each of them are responsible for a distinct aspect of the system.
...
These layers clearly separate responsibilities and facilitate maintainability. The UML class diagram Figure \ref{fig:class-diagram} illustrates these layers and the relationships between the classes,
...
In UML, this relationship is represented by a composition arrow pointing from the controller to the service class.
...
\begin{lstlisting}[caption={ReviewController -- Composition with ReviewService}, label={lst:reviewController}, numbers=left, firstnumber=1, breaklines=true, breakatwhitespace=true]
...
(lines 14--16), the ReviewController invokes the getReviewsByCuisine() method on its 
ReviewService object (line 16). The controller does not contain any business logic itself; 
it passes all such responsibilities to the service. Because the service object is initialized 
along with the controller (lines 10--11) and maintained as a private final field, the UML 
composition arrow in Figure \ref{fig:uml-controller} above accurately reflects the “has-a” relationship between the controller and the service.
...
\begin{lstlisting}[caption={CreateReviewRequest -- DTO for Review module}, label={lst:createReviewRequest}, numbers=left, firstnumber=1]
...
It ensures that only valid and necessary data is passed to the service layer (lines 5--14), 
allowing the internal entity objects, such as the Review entity, to remain decoupled from the API contract.
For example, the @NotNull annotation (lines 5, 8) ensures that none of the fields of the submitted data is empty.
...
\begin{lstlisting}[caption={ReviewController -- POST request handling with DTOs}, label={lst:reviewControllerPost}, numbers=left, firstnumber=1, breaklines=true, breakatwhitespace=true]
...
This dependency is short-lived. The controller uses the DTO objects only during the handling of each request (lines 4--7). 
Because the controller does not own these objects for its entire lifetime, UML diagram Figure \ref{fig:uml-controller} represents this relationship with a dependency arrow rather than a composition arrow.  \\
...
\begin{lstlisting}[caption={CuisineService -- Composition with Repositories and CacheService}, label={lst:cuisineServiceComposition}, numbers=left, firstnumber=1, breaklines=true, breakatwhitespace=true]
...
This means that the service objects owns the reference to the corresponding repository object for its lifetime and relies on it for any database interactions. 
These objects are injected via the constructor (lines 9--14) and exist for the lifetime of the service, reflecting strong ownership in UML. 
...
\begin{lstlisting}[caption={CuisineService -- dependency on DTO}, label={lst:cuisineServiceDto}, numbers=left, firstnumber=1, breaklines=true, breakatwhitespace=true]
...
\begin{lstlisting}[caption={CuisineResponse -- Output DTO}, label={lst:cuisineResponseDto}, numbers=left, firstnumber=1]
...
Unlike composition, the service does not maintain a permanent reference to the DTO object. It only uses the DTO briefly during method execution (lines 8--9 in Listing~\ref{lst:cuisineServiceDto}). 
...
The CuisineResponse DTO uses the Cuisine entity to send a respond back to the Cuisine Controller (lines 9--17 in Listing~\ref{lst:cuisineResponseDto}).
...
Because CuisineResponse only temporarily depends on the Cuisine entity during object creation, UML diagram Figure \ref{fig:uml-service} represents this relationship with a dependency arrow 
rather than a composition.  \\

\noindent \textbf{Repository Layer}
...
\begin{lstlisting}[caption={CuisineRepository -- JPA Repository Interface}, label={lst:cuisineRepository}, numbers=left, firstnumber=1, breaklines=true, breakatwhitespace=true]
...
\begin{lstlisting}[caption={CuisineService -- Retrieving cuisines by region}, label={lst:cuisineServiceGetByRegion}, numbers=left, firstnumber=1, breaklines=true, breakatwhitespace=true]
// CuisineService.java
...
We can see that the findByRegion\_RegionName method in the CuisineRepository (line 6)
...
The multiplicity 1 near Region and 0..* near Cuisine in the UML diagram Figure \ref{fig:uml-entity} shows that each cuisine must belong to exactly one region,
...
\begin{lstlisting}[caption={Region -- One-to-Many relationship with Cuisine}, label={lst:regionCuisine}, numbers=left, firstnumber=1, breaklines=true, breakatwhitespace=true]
// Region.java
...
In line 2, the '@OneToMany' annotation defines that a single Region can have many Cuisine objects,
with the 'mappedBy = "region"' attribute specifying that the Cuisine entity owns the relationship.
...
\begin{lstlisting}[caption={Cuisine -- Many-to-One relationship with Region}, label={lst:cuisineRegion}, numbers=left, firstnumber=1]
// Cuisine.java
@ManyToOne(fetch = FetchType.LAZY)
@JoinColumn(name = "region_id", nullable = false)
private Region region;
...
This establishes the many-to-one side of the region-cuisine relationship (line 3), allowing the cuisine entity to reference the region entity.
...
Each review has an id, a rating, a comment, and a timestamp. Each review is linked to a specific cuisine as shown in Listing \ref{lst:reviewCuisine}.
...
\begin{lstlisting}[caption={Review -- Many-to-One relationship with Cuisine}, label={lst:reviewCuisine}, numbers=left, firstnumber=1]
// Review.java
@ManyToOne(fetch = FetchType.LAZY)
...
the separation of concerns across layers, and how each component contributes to the overall functionality of the Local Cuisine App's system.  \\

\noindent \textbf{Coupling and Cohesion}

The structural relationships represented in the UML diagram not only define module boundaries but also reflect important software design qualities,
particularly coupling and cohesion.
These principles are essential for evaluating maintainability in a modular monolithic architecture.
Coupling refers to the degree of interdependence between modules \cite{GeeksforGeeks_2025}. 
In the presented UML diagram Figure \ref{fig:class-diagram}, different relationship types correspond 
to different levels of coupling strength.

Controller classes maintain a composition relationship with their 
corresponding services, 
and services maintain composition with repository interfaces. 
Composition represents stronger structural coupling, as the lifecycle 
of the composed object is bound to the owning class. 
However, this coupling is confined within a single module layer,
preserving architectural clarity and preventing cross-layer entanglement.

Within the domain model, Region has an aggregation relationship 
with Cuisine, while Cuisine has a composition relationship with Review. 
Aggregation indicates a weaker ownership relationship, 
as cuisines conceptually belong to a region but may exist independently 
at the persistence level. 
In contrast, composition represents tighter coupling, 
as reviews are inherently bound to a specific cuisine and cannot 
exist meaningfully without it. 
These structural couplings are intentional and reflect real-world 
hierarchical dependencies within the domain.

DTO classes such as CreateReviewRequest, ReviewResponse, 
and CuisineResponse are connected through dependency relationships. 
Dependency represents looser coupling, since these classes are used 
only as method parameters or return types without ownership or lifecycle control. 
This reduces structural rigidity and separates internal domain models from external API contracts.

Overall, the system achieves a balanced coupling strategy. 
Strong composition and aggregation relationships are confined to the domain and module layers where ownership and lifecycle dependencies are meaningful, 
while dependency relationships are used for external communication and data transfer. 
This combination ensures that modules are sufficiently decoupled for maintainability and testability, 
while allowing stronger coupling where necessary to preserve domain integrity. 
This supports a clear and coherent modular monolithic architecture.

Cohesion measures how strongly related the responsibilities within a module or class are, 
with higher cohesion improving maintainability and clarity \cite{GeeksforGeeks_2025}. 
Modules in this backend primarily demonstrate functional cohesion, 
as each module (Region, Cuisine, Review) focuses on a single domain concept and 
all components contribute directly to that responsibility. 
For instance, ReviewService, ReviewRepository, and ReviewController collectively 
handle review-related operations, without unrelated functionality.
By organizing the backend into domain-focused modules with clear boundaries, 
the system ensures high functional cohesion, 
reinforcing the modular monolithic architecture and coherent business capabilities.
