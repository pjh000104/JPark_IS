...
As shown in the entity relationship diagram \ref{fig:erd}, the regions table has a one-to-many relationship with the cuisines table, 
...
\begin{lstlisting}[caption={ReviewService -- Write-through caching example}, label={lst:reviewWriteThroughCache}, numbers=left, firstnumber=1]
...
\begin{lstlisting}[caption={ReviewService -- Cache-aside implementation for createReview}, label={lst:reviewCacheAside}, numbers=left, firstnumber=1]
public ReviewResponse createReview(CreateReviewRequest request) {
...
The following sequential diagram \ref{fig:sequential-diagram} shows the lifecycle of a backend request starting from a user. 
...
As shown in the sequential diagram \ref{fig:sequential-diagram}, the workflow begins when the user initiates a search action from the frontend interface.
...
\begin{lstlisting}[caption={CuisineController -- Handling incoming HTTP requests}, label={lst:cuisineController}, numbers=left, firstnumber=1]
@RestController
...
\begin{lstlisting}[caption={CuisineService -- Cache-aside lookup}, label={lst:cuisineServiceCache}, numbers=left, firstnumber=1]
...
\begin{lstlisting}[caption={CuisineService -- Database fallback}, label={lst:cuisineServiceDb}, numbers=left, firstnumber=1]
...
The following UML class diagram \ref{fig:class-diagram} provides a visual representation of the structural organization of the local cuisine application.
...
These layers clearly separate responsibilities and facilitate maintainability. The UML class diagram \ref{fig:class-diagram} illustrates these layers and the relationships between the classes,
...
\begin{lstlisting}[caption={ReviewController -- Composition with ReviewService}, label={lst:reviewController}, numbers=left, firstnumber=1]
...
composition arrow in the diagram \ref{fig:uml-controller} above accurately reflects the “has-a” relationship between the controller and the service.
...
\begin{lstlisting}[caption={ReviewController -- POST request handling with DTOs}, label={lst:reviewControllerPost}, numbers=left, firstnumber=1]
...
Because the controller does not own these objects for its entire lifetime, UML diagram \ref{fig:uml-controller} represents this relationship with a dependency arrow rather than a composition arrow.  \\
...
\begin{lstlisting}[caption={CuisineService -- Composition with Repositories and CacheService}, label={lst:cuisineServiceComposition}, numbers=left, firstnumber=1]
...
\begin{lstlisting}[caption={CuisineService -- dependency on DTO}, label={lst:cuisineServiceDto}, numbers=left, firstnumber=1]
...
Because CuisineResponse only temporarily depends on the Cuisine entity during object creation, UML diagram \ref{fig:uml-service} represents this relationship with a dependency arrow 
...
\begin{lstlisting}[caption={CuisineRepository -- JPA Repository Interface}, label={lst:cuisineRepository}, numbers=left, firstnumber=1]
...
\begin{lstlisting}[caption={CuisineService -- Retrieving cuisines by region}, label={lst:cuisineServiceGetByRegion}, numbers=left, firstnumber=1]
// CuisineService.java
...
The multiplicity 1 near Region and 0..* near Cuisine in the UML diagram \ref{fig:uml-entity} shows that each cuisine must belong to exactly one region,
...
\begin{lstlisting}[caption={Region -- One-to-Many relationship with Cuisine}, label={lst:regionCuisine}, numbers=left, firstnumber=1]
// Region.java
...
In the presented UML diagram \ref{fig:class-diagram}, different relationship types correspond 
